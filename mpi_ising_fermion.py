...
def main_routine(arg,c,alpha):
	...
	fname = "WDIR/Apr9/Data/ISING_ALT_L_%d_muh_%g_mul_%g_delta_%g_T_%g_cyc_%d_ncnf_%g_"%(L,mu_h,mu_l,delta,T,cyc,num_conf*mpi_size)
	...
	for k in range(num_conf):
		...
		mu_array  = np.asarray([np.cos(2*np.pi*sigma*nsite + alpha[k]) for nsite in range(L) ])
...
if __name__ == '__main__':	
	...
	if rank==0:
		...
		parser.add_argument("-mu_h","--mu_h",help="square pulse",default=.1,type=float,nargs='?')
		parser.add_argument("-mu_l","--mu_l",help="square pulse",default=.1,type=float,nargs='?')
