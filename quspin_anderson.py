...
fname = "WDIR/QUSPIN_NODIS_L_%d_A_%g_w_%g_mu0_%g_num_%d_tf_%g_dt_%g_"%(L,A,w,mu0,1,tf,dt)
# drive protocol
def drive(t,A,w):
	return  np.exp(-1j*A*np.sin(w*t))
...
# define basis
basis = spinless_fermion_basis_1d(L=L,Nf=3)
basis_dim = 120#int((fact(L))/(fact(L//2))**2)
...
H_img=hamiltonian(static,dynamic,basis=basis,dtype=np.float64)
tau = np.linspace(0.0,70.0,100)
psi_imag_t = H_img.evolve(psi0,tau[0],tau,iterate=True,atol=1E-12,rtol=1E-12,imag_time=True)
